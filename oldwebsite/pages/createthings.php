...
<div class="box">
		<div class="boxcontent" id="mybox">
            <h2>Title of Thing</h2>
            <div contenteditable="True" class="formtitle" id="formtitle"></div>
            <h2>Link to thing</h2>
            <div contenteditable="True" class="formlink" id="formlink"></div>
            <h2>Link to github</h2>
            <div contenteditable="True" class="formgithub" id="formgithub"></div>
            <h2>Content</h2>
...
function submitblog()
{
    ...
    var xhttp = new XMLHttpRequest();
    var myurl = "post.php?title="+mytitle+"&content="+mycontent+"&type="+type+"&mylink="+mylink+"&mygithub="+mygithub+"&role=Admin";
    xhttp.open("POST", myurl, true);
    xhttp.send();
    xhttp.onreadystatechange = function() {
        if (xhttp.readyState == 4 && xhttp.status == 200){
            mybox.innerHTML = "Submitted!";
            setTimeout(window.location.href = "?page=things", 2000);
        }};
